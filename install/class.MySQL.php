...
class MySQL {
	...
	private function Connect($persistant = false){
		...
		if(!$this->databaseLink){
   		    echo $this->lastError = 'Could not connect to server: ' . mysql_error($this->databaseLink);
   		    die();
			return false;
		}
		
		if(!$this->UseDB()){
			echo $this->lastError = 'Could not connect to database: ' . mysql_error($this->databaseLink);
			die();
			return false;
		}
		return true;
