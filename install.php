...
<title>Henfire Web Installer</title>
...
<div class="sub-header merchant-step-section" >
  <div class="main">
    <ul class="order-steps" style="margin-left:150px;">
      <li class="<?php echo $steps==1?"active current":'';?>">
...
<div class="page-right-sidebar">
  ...
  <?php $file_to_create="protected/common/config/main-local.php";?>
  <?php if (file_exists($file_to_create)):?>
  <p class="uk-text-danger">main-local.php file detected. Either you create the main-local.php manually or you have already install the application. </p>
  ...
  <?php
  $failed=0;
  $config_file="protected/common/config/";
  ...
  dump($host);
  die();*/
  if ( $host=="htdocs" || $host=="public_html" || $host=="/install"){
    $htaccess="
...
RewriteRule . /index.php [L]
  ...
  echo "<p class=\"uk-icon-chevron-right\"> Creating main-local.php(config) file..</p>";
  ...
  $config_file=str_replace("{table_prefix}",$table_prefix,$config_file);
	//Mail Settings
	$config_file=str_replace("{mail_host}",$data['mail_host'],$config_file);
	$config_file=str_replace("{mail_username}",$data['mail_username'],$config_file);
	$config_file=str_replace("{mail_password}",$data['mail_password'],$config_file);
	$config_file=str_replace("{mail_port}",$data['mail_port'],$config_file);
  $file_to_create="protected/common/config/main-local.php";
  ...
  createFile($file_to_create,$config_file);
 echo "<div>Creating main-local.php [done]</div>";
  ?>
  ...
  <form class="uk-form uk-form-horizontal forms" id="forms" method="POST">
   ...
   <input type="text" name="db_table_prefix" class="uk-form-width-large" placeholder="mh_" value="<?php echo isset($data['db_table_prefix'])?$data['db_table_prefix']:''?>" >
   </div>

<h2>Mail Server</h2>
<div class="uk-form-row">
<label class="uk-form-label">Mail Host</label>
<input type="text" name="mail_host" class="uk-form-width-large" value="<?php echo isset($data['mail_host'])?$data['mail_host']:''?>">
</div>

<div class="uk-form-row">
<label class="uk-form-label">Mail Username</label>
<input type="text" name="mail_username" class="uk-form-width-large" value="<?php echo isset($data['mail_username'])?$data['mail_username']:''?>" >
</div>

<div class="uk-form-row">
<label class="uk-form-label">Mail Password</label>
<input type="text" name="mail_password" class="uk-form-width-large" value="<?php echo isset($data['mail_password'])?$data['mail_password']:''?>" >
</div>

<div class="uk-form-row">
<label class="uk-form-label">Port (SSL)</label>
<input type="text" name="mail_port" class="uk-form-width-large" placeholder="465" value="<?php echo isset($data['mail_port'])?$data['mail_port']:''?>" >
</div>

	<div class="uk-form-row">
	<label class="uk-form-label"></label>
	<input type="submit" value="Install Db" class="uk-button uk-form-width-medium uk-button-success">
	</div>
  ...
  if ( $validator->validate()){
      ...
      $table_prefix=!empty($table_prefix)?$table_prefix:"mh_";
      ...
      $params=array(
       'sitename'=>$data['website_title'],
	     'email'=>$data['website_contact_email'],
       'currency'=>$admin_currency_set[0]
      );

     echo "<p class=\"uk-icon-chevron-right\"> Inserting user ...</p>";
      $truncate="TRUNCATE TABLE ".$table_prefix."settings";
      $db->raw($truncate);
      $db->create( $table_prefix."settings", $params);
      echo "<div> [done]</div>";
			...
			//Make Admin setup
			$config_file2=file_get_contents($current_dir."/install/main.php");
			$config_file2=str_replace("{admin_user}",$data['username'],$config_file2);
		  $file_to_create2="protected/common/config/main.php";
			if (file_exists($file_to_create2)){
		  	  @unlink($file_to_create2);
		  }
		  createFile($file_to_create2,$config_file2);
    ...
    'status'=>'10',
		'currency'=>$admin_currency_set[0],
		'country_code' => $date['admin_country_set'],
      );
      echo "<p class=\"uk-icon-chevron-right\"> Inserting user ...</p>";
      ...
      } else $admin_url="http://".$_SERVER['HTTP_HOST']."";
      ?>
